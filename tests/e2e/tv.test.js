...
//
// Determinism contract (2026-08-05): every case runs inside testCase() and
// waits are condition-based (.prayer-grid visibility + network-quiet settle),
// never fixed "hope" sleeps.
// ---------------------------------------------------------------------------
...
async function fetchTheme(slug) {
  const resp = await fetch(`${cfg.api}/api/v1/masjids/${slug}`, { signal: AbortSignal.timeout(15000) });
...
await testCase(t, 'TV-03', async () => {
  const context = await newContext(browser);
  const page = await context.newPage();
  const b = collectPage(page, cfg);

  await gotoPage(page, b, `${cfg.tv}/display/${SLUG_A}`, { expectSelector: '.prayer-grid' });
...
await testCase(t, 'TV-04', async () => {
  ...
  const context = await newContext(browser);
  const page = await context.newPage();
  await page.goto(`${cfg.tv}/display/${SLUG_UNKNOWN}`, { waitUntil: 'load' });
...
await testCase(t, 'TV-05', async () => {
  ...
  await gotoPage(page, b, `${cfg.tv}/display/${SLUG_B}`, { expectSelector: '.prayer-grid' });
  await gotoPage(page, b, `${cfg.tv}/display/${SLUG_A}`, { expectSelector: '.prayer-grid' });
