...
- **`style_options.hideHomeNav` (2026-08-13)**: boolean — hides the Home tab (desktop header + mobile bottom nav) because the header logo/name already links home. Admin toggle lives in the Navigation settings page ("Hide the Home tab"; it persists via the profile API, sending the full `style_options` object). Default `false`. See `docs/nav-config.md` §2.5.
